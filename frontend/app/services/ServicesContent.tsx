...
const CATEGORIES: { id: ServiceCategory | 'all'; icon: string; color: string; bg: string }[] = [
  { id: 'all', icon: 'badge', color: 'text-secondary', bg: 'bg-secondary/10' },
  { id: 'identity_civil', icon: 'fingerprint', color: 'text-info', bg: 'bg-info/10' },
  { id: 'education_skills', icon: 'school', color: 'text-purple-600', bg: 'bg-purple-50' },
  { id: 'health_welfare', icon: 'local_hospital', color: 'text-success', bg: 'bg-success/10' },
  { id: 'business_trade', icon: 'storefront', color: 'text-amber-600', bg: 'bg-amber-50' },
  { id: 'housing_land', icon: 'home_work', color: 'text-error', bg: 'bg-error/10' },
];
...
export default function ServicesContent() {
  ...
  return (
    <div className="max-w-container-max mx-auto w-full flex flex-col md:flex-row">
      {/* Sidebar — sticky, stays in view while scrolling */}
      <aside className="hidden md:flex flex-col w-72 bg-surface border-r border-outline-variant/50 shrink-0 sticky top-[120px] self-start h-[calc(100vh-120px)] overflow-y-auto">
        <div className="py-6 px-4">
          ...
          <div className="mb-8">
            ...
            <nav className="flex flex-col gap-1">
              {CATEGORIES.map(({ id, icon, color, bg }) => (
                <button
                  key={id}
                  type="button"
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${
                    category === id ? bg : 'bg-neutral-50'
                  }`}>
                    <Icon name={icon} filled={category === id} size={18} className={category === id ? color : 'text-on-surface-variant'} />
                  </div>
      ...
      {/* Main content — flows naturally so footer appears after all services */}
      <main className="flex-1 bg-background min-w-0">
        {/* Sticky header matching sidebar */}
        <div className="sticky top-0 z-10 bg-background border-b border-outline-variant/50 px-5 md:px-8 py-5">
          <FadeIn>
